...
public class WebsiteData {
	...
	public WebsiteData(Context ctx, String cid) {
		this.context = ctx;
		this.customerid = cid;
	...
	public void add_Website() {
		String response = Webservice.Website(customerid, "android");
		...
		try {
			...
			if (jo.getString("status").equals("success")) {
				...
				for (int i = 0; i < ja.length(); i++) {
					...
					JSONObject jo3 = (JSONObject) jo2.get("tbl_module_website");

					jo3.getString("module_website_id");
					...
					dba.insertWebsite(jo3.getString("module_website_id"),
							jo3.getString("customer_id"),
					...
					JSONArray jarr = (JSONArray) jo2
							.get("tbl_module_website_detail");

					for (int j = 0; j < jarr.length(); j++) {

						JSONObject jo4 = (JSONObject) jarr.get(j);

						dba.insertWebsiteDetails(
								jo4.getString("module_website_detail_id"),
								jo4.getString("module_website_id"),
		...
		try {
			...
			if (jo.getString("status").equals("success")) {
				...
				for (int i = 0; i < ja.length(); i++) {
					...
					list.add(jo3.getString("module_website_id"));

					Cursor c = dba.getWebsiteDate(jo3
							.getString("module_website_id"));
					...
					if (c.getCount() > 0) {
						...
						if (Util.compareDateTime(c.getString(0),
							...
							for (int j = 0; j < jarr.length(); j++) {
								...
								Cursor cur_hd = dba.getWebsiteDetailDate(jo4
										.getString("module_website_detail_id"));
							...
							String[] arr1 = list1.toArray(new String[list1
									.size()]);
							DeleteDetailTableRecords("tbl_Website_Details",
									"RecordID", "WebsiteID",
				...
				DeleteRecord("tbl_Module_Website", "WebsiteID", arr);
