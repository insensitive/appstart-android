...
public class SocialMediaData {
	...
	public SocialMediaData(Context ctx, String cid) {
		this.context = ctx;
		this.customerid = cid;
	...
	public void add_SocialMedia() {
		String response = Webservice.SocialMedia(customerid, "android");
			...
			if (jo.getString("status").equals("success")) {
				...
				for (int i = 0; i < ja.length(); i++) {
					...
					JSONObject jo3 = (JSONObject) jo2
							.get("tbl_module_social_media");

					dba.insertSocialMedia(
							jo3.getString("module_social_media_id"),
					...
					JSONArray jarr = (JSONArray) jo2
							.get("tbl_module_social_media_detail");

					for (int j = 0; j < jarr.length(); j++) {

						JSONObject jo4 = (JSONObject) jarr.get(j);

						String img_url = jo4.getString("icon_path");
						...
						dba.insertSocialMediaDetails(
								jo4.getString("module_social_media_detail_id"),
								jo4.getString("module_social_media_id"),
								jo4.getString("language_id"),
								jo4.getString("social_media_type_id"),
								jo4.getString("title"), jo4.getString("url"),
								img_name, jo4.getString("last_updated_by"),
	...
	// ---social media sync module

	public void SocialMedia_sync_module() {
		String response = Webservice.SocialMedia(customerid, "android");
		...
		try {
			...
			if (jo.getString("status").equals("success")) {
				...
				for (int i = 0; i < ja.length(); i++) {
					...
					list.add(jo3.getString("module_social_media_id"));

					Cursor c = dba.getSocialMediaDate(jo3
							.getString("module_social_media_id"));
					...
					if (c.getCount() > 0) {
						...
						if (Util.compareDateTime(c.getString(0),
							...
							for (int j = 0; j < jarr.length(); j++) {
								...
								Cursor cur_hd = dba
										.getSocialMediaDetailDate(jo4
												.getString("module_social_media_detail_id"));
							...
							String[] arr1 = list1.toArray(new String[list1
									.size()]);
							DeleteDetailTableRecords("tbl_SocialMedia_Details",
									"RecordID", "SocialMediaID",
									jo3.getString("module_social_media_id"),
									arr1);
						...
						for (int j = 0; j < jarr.length(); j++) {
							...
							dba.insertSocialMediaDetails(
									...
									jo4.getString("language_id"), jo4
											.getString("social_media_type_id"),
									jo4.getString("title"), jo4
											.getString("url"), img_name, jo4
				...
				DeleteRecord("tbl_SocialMedia", "SocialMediaID", arr);
